...
class FrequencyTimeDomainORM(ORM):
    """
    Reward function based on the presence of Frequency Domain and Time Domain tags in thinking content.
    - Neither present: 0
    - One present: 0.5
    - Both present: 1.0 (maximum reward)
    """
    ...
    def __call__(self, completions, solution=None, **kwargs) -> List[float]:
        ...
        Args:
            completions (list[str]): Generated outputs
            solution (list[str]): Ground truth solutions (optional)
            
        Returns:
            list[float]: Reward scores (0-1.0)
        """
        rewards = []
        ...
        for completion in completions:
            ...
            try:
                thinking_match = re.search(r'<think>(.*?)</think>', completion, re.DOTALL)
                if thinking_match:
                    thinking_content = thinking_match.group(1).strip()
                    has_frequency = '<Frequency Domain>' in thinking_content
                    has_time = '<Time Domain>' in thinking_content
                    
                    if has_frequency and has_time:
                        reward = 1.0  
                    elif has_frequency or has_time:
                        reward = 0.5  
                    else:
                        reward = 0.0  
                ...
                logger.warning(f"Error processing completion for reward: {e}")
                rewards.append(0.0)
        
        return rewards


orms['ft_reward'] = FrequencyTimeDomainORM
